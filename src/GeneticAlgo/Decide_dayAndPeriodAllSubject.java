 ...
 * @author Nakata
 */
public class Decide_dayAndPeriodAllSubject extends Decide_faculty implements
		iDayPeriod, iTimeTable {

	private final String TIME_TABLE3_PATH = "C:\\Users\\NakataKodai\\Documents\\AllTimeTableFile\\";// 時間割のパス
	 ...
	 * コンストラクタ
	 */
	public Decide_dayAndPeriodAllSubject() {
		super();
	 ...
	 * @return 評価値の合計
	 */
	private long getSumEvaluationValue() {
	...
	private void writeDayAndPeriodFile3(int count) {
		...
		for (int candidate = 0; candidate < CANDIDATE_NUM; candidate++) {
			...
			PrintWriter output;
			output = FileIO.writeFile(TIME_TABLE3_PATH + (count + 1) + "回目\\"
	...
	private void readStudentDataFile() {
		...
		BufferedReader input = FileIO.readFile(TIME_TABLE3_PATH
				+ STUDENT_DATA_FILE);
	...
	private void readFacultyFile1() {
		...
		BufferedReader input = FileIO
				.readFile(TIME_TABLE3_PATH + FACULTY1_NAME);
	...
	private void readFacultyFile2() {
		...
		BufferedReader input = FileIO
				.readFile(TIME_TABLE3_PATH + FACULTY2_NAME);
	...
	private void readFacultyFile3() {
		...
		BufferedReader input = FileIO
				.readFile(TIME_TABLE3_PATH + FACULTY3_NAME);
