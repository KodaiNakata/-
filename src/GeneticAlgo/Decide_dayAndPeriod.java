...
public class Decide_dayAndPeriod extends Decide_faculty implements iDayPeriod,
	...
	private void updateStudentData(int candidate, boolean doUpdate) {
	...
	private void exeGeneticAlgo() {
		...
		for (int num = 0; num < CHECK_NUM; num++) {
			...
			// 新しい評価値以下のとき
				...
				for (int candidate = 0; candidate < CANDIDATE_NUM; candidate++) {
					updateStudentData(candidate, false);
					resetNewTimeTableData3(candidate, true);// 時間割をリセットする
				}
			}
					...
					for (int period = 1; period <= MAX_PERIOD; period++) {
						...
						for (int grade = 1; grade <= MAX_GRADE; grade++) {
							...
							if (0 <= mutationNumber) {
								...
								// int addProbablity =
								// getAddProbablityFromStudentData(candidate,
								// semester,
								// grade, day);// 追加の確率
	...
	private int getRandomPeriod(int candidate, int day_of_week, int studentNum) {
		...
		for (int period = 1; period <= MAX_PERIOD; period++) {
			...
			int number = f_NewStudentData.get(studentNum).getDayPeriodNumber(
	...
	private void writeDayAndPeriodFile3() {
		...
		for (int candidate = 0; candidate < CANDIDATE_NUM; candidate++) {
			...
			PrintWriter output;
			output = FileIO.writeFile(TIME_TABLE_PATH + "room_3_"
					+ (candidate + 1) + ".csv", false);
	...
	private void writeDayAndPeriodFirstFile3() {
		...
		for (int candidate = 0; candidate < CANDIDATE_NUM; candidate++) {
			PrintWriter output;
			output = FileIO.writeFile(TIME_TABLE_PATH + "roomFirst_3_"
					+ (candidate + 1) + ".csv", false);
	...
	private void writeStudentDataBestFile() {
		...
		PrintWriter output;
		output = FileIO.writeFile(TIME_TABLE_PATH + "studentBestData.csv",
				false);
	...
	private void writeStudentDataFile() {
		...
		for (int candidate = 0; candidate < CANDIDATE_NUM; candidate++) {
			...
			PrintWriter output;
			output = FileIO.writeFile(TIME_TABLE_PATH + "studentData"
					+ (candidate + 1) + ".csv", false);
	...
	private void writeFirstStudentDataBestFile() {
		...
		PrintWriter output;
		output = FileIO.writeFile(TIME_TABLE_PATH + "studentFirstBestData.csv",
				false);
	...
	private void writeFirstStudentDataFile() {
		...
		for (int candidate = 0; candidate < CANDIDATE_NUM; candidate++) {
			...
			PrintWriter output;
			output = FileIO.writeFile(TIME_TABLE_PATH + "studentFirstData"
					+ (candidate + 1) + ".csv", false);
